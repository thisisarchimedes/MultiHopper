...
contract Deploy is BaseScript {
    ...
    function run() public broadcaster returns (address, address, address, address) {
        address deployerAddress = vm.envAddress("SENDER_ADDRESS");
        // console2.log("Deployer Address: %s", deployerAddress);
        multiPoolStrategyFactory = new MultiPoolStrategyFactory(deployerAddress);
        // console2.log("MultiPoolStrategyFactory: %s", address(multiPoolStrategyFactory));
        //// test underlying WETH contract works
        // console2.log("deployer eth balance %s", deployerAddress.balance / 1e18);
        IWETH(payable(UNDERLYING_TOKEN)).deposit{ value: 1e18 }();
        uint256 deployerWETHBalance = IERC20(payable(UNDERLYING_TOKEN)).balanceOf(deployerAddress);
        ...
        // ethZapper.depositETH{ value: 10e18 }(10e18, deployerAddress);
        ...
        return (
            ...
            address(ethZapper),
            address(multiPoolStrategyFactory)
        );
    }
}
